...
const MarkCard = ({ user, isLoading, handleSubmit, attendanceDate }) => {
  ...
  const isTimeAfter930AM =
    checkInTime >= dayjs(attendanceDate).set("hour", 9).set("minute", 30);
  ...
  const onSubmit = async (values) => {
    ...
    const date = formatDate(attendanceDate, "yyyy-MM-dd");
    const time = formatTime(new Date(checkInTime), "HH:mm:ss");
